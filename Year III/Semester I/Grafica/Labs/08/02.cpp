...
float const U_MIN = 0, U_MAX = 2 * PI, V_MIN = -150, V_MAX = 0;
// (2) numarul de paralele/meridiane, de fapt numarul de valori ptr parametri
int const NR_PARR = 10, NR_MERID = 10;
...
void CreateVBO(void)
{
    ...
    for (int merid = 0; merid < NR_MERID; merid++)
    {
        for (int parr = 0; parr < NR_PARR + 1; parr++)
        {
            ...
            float x_vf = 0.5 * v * cosf(u); // coordonatele varfului corespunzator lui (u,v)
            float y_vf = 0.5 * v * sinf(u);
            float z_vf = v;
            ...
            // Colors[index] = glm::vec3(0.5f, 0.5f, 0.5f);
            Indices[index] = index;
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("Lab 08 Ex 02");     //  Creeaza fereastra de vizualizare, indicand numele acesteia;
