...
void CreateVBO(void)
{
    ...
    for (int merid = 0; merid < NR_MERID; merid++)
    {
        for (int parr = 0; parr < NR_PARR + 1; parr++)
        {
            ...
            float grey = 0.66f + cosf(u);
            grey = glm::clamp(grey, 0.55f, 0.75f);
            Colors[index] = glm::vec3(grey, grey, grey);
            Indices[index] = index;
...
void drawPlanet(int codColValue)
{
    codCol = codColValue;
...
void RenderFunction(void)
{
    ...
    translatePlanet = glm::translate(glm::mat4(1.0f), glm::vec3(125.0, 0.0, 0.0));

    scaleSatelite = glm::scale(glm::mat4(1.0f), glm::vec3(0.2, 0.2, 0.2));
    rotateSateliteAxis = glm::rotate(glm::mat4(1.0f), (float)0.001 * timeElapsed, glm::vec3(0.0, 1.0, 0.0));
    rotateSatelite = glm::rotate(glm::mat4(1.0f), (float)0.002 * timeElapsed, glm::vec3(-0.1, 1.0, 0.0));
    translateSatelite = glm::translate(glm::mat4(1.0f), glm::vec3(40.0, 0.0, 0.0));
    ...
    mvStack.top() *= rotateSatelite;
    mvStack.top() *= translateSatelite;
    mvStack.top() *= rotateSateliteAxis;
    mvStack.top() *= scaleSatelite;
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("Lab 08 Ex 01");       //  Creeaza fereastra de vizualizare, indicand numele acesteia;
