...
const int NR_MERID_CILINDRU = 25, RADIUS_CILINDRU = 25;

// Elemente pentru reprezentarea cilindrului
const int NR_MERID_CON = 25, RADIUS_CON = 75, HEIGHT_CON = 175;
...
void CreateVAO1(void)
{
    ...
    glm::vec4 Vertices[NR_MERID_CILINDRU * 2];
    glm::vec3 Colors[NR_MERID_CILINDRU * 2];
    GLushort Indices[NR_MERID_CILINDRU * 6];

    for (int merid = 0; merid < NR_MERID_CILINDRU; ++merid)
    {
        // implementarea reprezentarii parametrice
        float u = 2 * PI * merid / NR_MERID_CILINDRU;

        float x_vf = RADIUS_CILINDRU * cosf(u);
        ...
        // baza superioara
        Vertices[NR_MERID_CILINDRU + merid] = glm::vec4(x_vf, y_vf, 0.0, 1.0);
        Colors[NR_MERID_CILINDRU + merid] = glm::vec3(0.445f, 0.197f, 0.035);
        Indices[NR_MERID_CILINDRU + merid] = NR_MERID_CILINDRU + merid;


        // indicii pentru fetele laterale
        Indices[2 * NR_MERID_CILINDRU + 4 * merid] = merid;
        Indices[2 * NR_MERID_CILINDRU + 4 * merid + 1] = NR_MERID_CILINDRU + merid;
        Indices[2 * NR_MERID_CILINDRU + 4 * merid + 2] = NR_MERID_CILINDRU + merid + 1;
        Indices[2 * NR_MERID_CILINDRU + 4 * merid + 3] = merid + 1;
    };

    Indices[6 * NR_MERID_CILINDRU - 2] = NR_MERID_CILINDRU;
    Indices[6 * NR_MERID_CILINDRU - 1] = 0;
...
void CreateVAO2(void)
{
    ...
    //  Matricele pentru varfuri, culori, indici
    glm::vec4 Vertices[NR_MERID_CON + 1];
    glm::vec3 Colors[NR_MERID_CON + 1];
    GLushort Indices[4 * NR_MERID_CON];

    // Se creeaza baza conului (un cerc)
    for (int i = 0; i < NR_MERID_CON; ++i)
    {
        float u = 2 * PI * i / NR_MERID_CON;

        float x = RADIUS_CON * cos(u);
        float y = RADIUS_CON * sin(u);
        float z = 0;
    ...
    Vertices[NR_MERID_CON] = glm::vec4(0, 0, HEIGHT_CON, 1);
    Colors[NR_MERID_CON] = glm::vec3(0, 0.6, 0);

    // Se creeaza indicii pentru fetele conului
    for (int i = 0; i < NR_MERID_CON; ++i)
    {
        Indices[NR_MERID_CON + 3 * i] = NR_MERID_CON;
        Indices[NR_MERID_CON + 3 * i + 1] = i;
        Indices[NR_MERID_CON + 3 * i + 2] = (i + 1) % NR_MERID_CON;
    }
...
void RenderFunction(void)
{
    ...
    glDrawElements(GL_TRIANGLE_FAN, NR_MERID_CILINDRU, GL_UNSIGNED_SHORT, 0);

    // desenarea bazei inferioare
    glDrawElements(GL_TRIANGLE_FAN, NR_MERID_CILINDRU, GL_UNSIGNED_SHORT, (GLvoid*)(NR_MERID_CILINDRU * sizeof(GLushort)));

    // desenarea fetelor laterale
    glDrawElements(GL_QUADS, 4 * NR_MERID_CILINDRU, GL_UNSIGNED_SHORT, (GLvoid*)(2 * NR_MERID_CILINDRU * sizeof(GLushort)));

    // CONUL
    glBindVertexArray(VaoId2);

    // desenarea bazei
    glDrawElements(GL_TRIANGLE_FAN, NR_MERID_CON, GL_UNSIGNED_SHORT, 0);

    // desenarea fetelor laterale
    for (int i = 0; i < NR_MERID_CON; ++i)
    {
        glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, (void*)(sizeof(GLushort) * (NR_MERID_CON + 3 * i)));
    }
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("Lab 08 Ex 03");      //  Creeaza fereastra de vizualizare, indicand numele acesteia;
