...
substitute [] t = t
substitute ((Equ (Variable v) x):ls) t = substitute ls (subst t v x)
substitute (x:xs) t = substitute xs t
...
vara (AtomicRel _ ts) = var (FuncSym "" ts)

-- substitution for an atomic formula
substitutea :: [Equ] -> AtomicRel -> AtomicRel
substitutea e (AtomicRel s l) = AtomicRel s (map (substitute e) l)

-- unification for two atomic formulas
unifya :: AtomicRel -> AtomicRel -> AllResult
unifya (AtomicRel s l) (AtomicRel s' l')
    | s == s' = unify (zipWith Equ l l')
    | otherwise = Failure
...
varngl :: Goal -> [String]
varngl g = filter (\x -> not . elem x . vargl $ g) $ freshvarlist

-- renaming of a clause according to a goal
renclause :: Goal -> Clause -> Clause
renclause g c = substitutecl (zipWith Equ (map Variable . varcl $ c) (map Variable . varngl $ g)) c
