...
instance Show Term where
    show (Variable v) = v
    show (FuncSym _ (x:y:_)) = "(" ++ show x ++ " -> " ++ show y ++ ")"
...
annotate_aux (Lam x y) (h:t) = (ALam x h z, l)
    where (z, l) = annotate_aux y t
annotate_aux (App x y) l = (AApp x' y', l'')
    where (x', l') = annotate_aux x l
          (y', l'') = annotate_aux y l'
...
constraints_aux (AVar x) (l:ls) gm = ([Equ (Variable l) . Variable . findx $ gm], ls)
    where findx ((y, z):t) = if x == y then z else findx t
constraints_aux (ALam x t y) (l:ls) gm = ((Equ (Variable l) (FuncSym "Arrow" [Variable t, Variable . head $ ls])):y', left)
    where (y', left) = constraints_aux y ls ((x, t):gm)
constraints_aux (AApp x y) (l:ls) gm = (((Equ (Variable . head $ ls) (FuncSym "Arrow" [Variable . head $ ls', Variable l])):c0) ++ c1, ls'')
    where (c0, ls') = constraints_aux x ls gm
          (c1, ls'') = constraints_aux y ls' gm
