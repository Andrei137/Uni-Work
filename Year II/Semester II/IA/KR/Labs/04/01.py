...
class Graf:
    ...
    def minimax(self, nod, depth, max_player):
        ...
        if max_player:
            ...
            for succesor in succesori:
                new_value, new_best = self.minimax(succesor, depth - 1, False)
                if new_value > value:
                    value = new_value
                    best = succesor
        else:
            value = float('inf')
            for succesor in succesori:
                new_value, new_best = self.minimax(succesor, depth - 1, True)
                if new_value < value:
                    value = new_value
                    best = succesor
        return value, best
    ...
    def print_sol(self):
        ...
        while not self.scop(nod.informatie):
            value, nod = self.minimax(nod, depth, max_player)
            depth -= 1
