...
class Info:
    def __init__(self, canibali, misionari, pozitieBarca):
        self.canibali = canibali  # dreapta
        self.misionari = misionari  # stanga
    ...
    def __str__(self):
        return formatString(self.misionari, Nod.NC - self.canibali, ", ") + " | " + \
               formatString(Nod.NM - self.misionari, self.canibali, ", ") + \
    ...
    def validState(self):
        if self.misionari != Nod.NM and self.canibali > Nod.NM - self.misionari:  # malul drept
            return False
        return True


class Nod:
    with open('Input/input_ex4.txt') as fin:
        NM = int(fin.readline())
        NC = int(fin.readline())
        M = int(fin.readline())

    def __init__(self, canibali=NC, misionari=NM, pozitieBarca=0, parinte=None, succesori=[]):
    ...
    def findSuccesori(self):
        for nrMisionari in range(1, self.M + 1):
            for nrCanibali in range(0, nrMisionari):
                if nrMisionari + nrCanibali > Nod.M:
                    continue
                if self.informatie.pozitieBarca == 0:  # stanga -> dreapta
                    ...
                    canibali = self.informatie.canibali - nrCanibali
                    misionari = self.informatie.misionari + nrMisionari
                if canibali < 0 or canibali > self.NC or misionari < 0 or misionari > self.NM:
                    continue
